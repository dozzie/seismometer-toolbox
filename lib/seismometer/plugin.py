  ...
  def load(self, name, file = None):
    ...
    **NOTE**: Specifying a ``name`` under non-existent hierarchy may cause
    ...
    if file is None:
      ...
      # `plugin' now contains top-level module, hence some digging required
      for n in name.split('.')[1:]:
        plugin = getattr(plugin, n)
    else:
      if '.' in name:
        # try to load "parent" module, so imp.load_source() won't complain
        try:
      ...
      # we need a file name under tmpdir, so *.pyc file lands there; let's lie
      # a little to the interpreter
